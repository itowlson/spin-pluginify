...
impl PluginifyCommand {
    ...
    pub fn run_local(&self) -> Result<(), Error> {
        let file = self
            .file
            .clone()
    ...
    pub fn run_merge(&self) -> Result<(), Error> {
    ...
    pub fn as_merge_set(&self, path: &PathBuf) -> Option<MergeFiles> {
    ...
    pub fn package(&self, ps: &PackagingSettings) -> Result<plugin_manifest::PluginPackage, Error> {
        let os = self
            .os_override
            .clone()
...
pub fn infer_package_path(ps: &PackagingSettings) -> PathBuf {
    ps.package.clone()
}
...
pub fn file_digest_string(path: &PathBuf) -> Result<String, Error> {
